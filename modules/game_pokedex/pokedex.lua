...
function doCreatePokedex(protocol, opcode, buffer)
  ...
  for pokeId = 1, 56 do
    ...
    pokemon.pokeDesc = string.explode(string.explode(buffer, ';')[pokeId], '|')[4]
    pokemon.pokeAttack = string.explode(string.explode(buffer, ';')[pokeId], '|')[5]
    pokemon.pokeDefense = string.explode(string.explode(buffer, ';')[pokeId], '|')[6]
    pokemon.pokeSpAttack = string.explode(string.explode(buffer, ';')[pokeId], '|')[7]
    pokemon.pokeVitality = string.explode(string.explode(buffer, ';')[pokeId], '|')[8]
    pokemon.pokeType1 = string.find(string.explode(string.explode(buffer, ';')[pokeId], '|')[9], '/') and string.explode(string.explode(string.explode(buffer, ';')[pokeId], '|')[9], '/')[1] or string.explode(string.explode(buffer, ';')[pokeId], '|')[9]
    pokemon.pokeType2 = string.find(string.explode(string.explode(buffer, ';')[pokeId], '|')[9], '/') and string.explode(string.explode(string.explode(buffer, ';')[pokeId], '|')[9], '/')[2] or nil
    ...
    pokemon.onFocusChange = function()
      ...
      if focused.pokeName == '??????' then
        ...
        removeEvent(event)
        event = scheduleEvent(function() doChangePokedexInfo(focused.pokeName, focused.pokeId, focused.pokeLevel, focused.pokeDesc, focused.pokeAttack, focused.pokeDefense, focused.pokeSpAttack, focused.pokeVitality, focused.pokeType1, focused.pokeType2) end, 250)
      end
    end
